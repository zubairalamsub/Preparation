...
import { ApiService, OOPTopic } from '../../services/api.service';
...
@Component({
  ...
  template: `
    <div>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <h1 style="font-size: 1.75rem; font-weight: 700;">OOP Concepts</h1>
        <div style="display: flex; gap: 0.5rem;">
          @if (topics.length === 0) {
            <button class="btn btn-success" (click)="seedTopics()">Load OOP Topics with Lessons</button>
          }
      ...
      @if (topics.length === 0) {
        <div class="card"><div class="empty-state"><div class="empty-state-icon">&#128736;</div><div>No topics. Click "Load OOP Topics with Lessons" to start!</div></div></div>
      } @else {
          ...
          @for (topic of topics; track topic.id) {
            <div class="card">
              ...
              <div style="margin-bottom: 1rem;">
                <div style="font-size: 0.75rem; color: var(--text-secondary); margin-bottom: 0.25rem;">Confidence</div>
                <div style="display: flex; gap: 0.25rem;">@for (i of [1,2,3,4,5]; track i) { <div style="width: 24px; height: 8px; border-radius: 4px;" [style.background]="i <= topic.confidenceLevel ? '#6366f1' : 'var(--bg-dark)'"></div> }</div>
              </div>
              ...
              <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                @if (topic.lesson) {
                  <button class="btn btn-success" style="flex: 1;" (click)="openLessonModal(topic)">View Lesson</button>
                }
      ...
      @if (showModal) {
        <div class="modal-backdrop" (click)="closeModal()">
          <div class="modal" (click)="$event.stopPropagation()">
            <h2 class="modal-title">{{ editingTopic ? 'Edit' : 'Add' }} OOP Topic</h2>
            <form (ngSubmit)="saveTopic()">
              ...
              <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 1.5rem;">
                ...
                <button type="submit" class="btn btn-primary">{{ editingTopic ? 'Update' : 'Add' }}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div class="modal-backdrop" (click)="closeReviewModal()">
          <div class="modal" (click)="$event.stopPropagation()">
            ...
            <form (ngSubmit)="saveReview()">
              ...
              <div class="form-group"><label class="form-label">Confidence (1-5)</label><div style="display: flex; gap: 0.5rem;">@for (i of [1,2,3,4,5]; track i) { <button type="button" class="btn" [class.btn-primary]="reviewData.confidenceLevel === i" [class.btn-secondary]="reviewData.confidenceLevel !== i" (click)="reviewData.confidenceLevel = i">{{ i }}</button> }</div></div>
            ...
            <div style="overflow-y: auto; flex: 1; padding: 1rem 0;">
              ...
              @if (lessonTopic.codeExample) {
                <div style="margin-top: 2rem;">
                  <h3 style="color: #10b981; margin-bottom: 1rem;">Code Example</h3>
  ...
  styles: [`
    ...
    .lesson-content h1 { font-size: 1.75rem; font-weight: 700; margin: 1.5rem 0 1rem; color: #818cf8; }
    .lesson-content h2 { font-size: 1.4rem; font-weight: 600; margin: 1.5rem 0 0.75rem; color: #a5b4fc; }
    .lesson-content h3 { font-size: 1.15rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: #c7d2fe; }
    ...
    .lesson-content th, .lesson-content td { border: 1px solid var(--border-color); padding: 0.5rem; text-align: left; }
    .lesson-content th { background: var(--bg-dark); }
    .lesson-content strong { color: #fbbf24; }
  `]
})
export class OopComponent implements OnInit {
  topics: OOPTopic[] = []; allTopics: OOPTopic[] = []; showModal = false; showReviewModal = false; showLessonModal = false;
  editingTopic: OOPTopic | null = null; selectedTopic: OOPTopic | null = null; lessonTopic: OOPTopic | null = null; showFavoritesOnly = false;
  filters = { category: '', status: '' };
  allCategories = ['SOLID Principles', 'Four Pillars', 'Concepts', 'Relationships', 'Advanced', 'Design Principles'];
  formData: Partial<OOPTopic> = this.getEmptyForm();
  reviewData = { status: 'Understood', confidenceLevel: 3 };

  constructor(private api: ApiService) {}
  ngOnInit() { this.loadTopics(); }

  loadTopics() {
    this.api.getOOPTopics(this.filters).subscribe(topics => {
      this.allTopics = topics;
      this.topics = this.showFavoritesOnly ? topics.filter(t => t.isFavorite) : topics;
    });
  }
  ...
  getEmptyForm(): Partial<OOPTopic> { return { title: '', category: '', difficulty: 'Medium', status: 'NotStarted', confidenceLevel: 1, keyConcepts: '', codeExample: '', tags: [], isFavorite: false }; }

  openModal() { this.editingTopic = null; this.formData = this.getEmptyForm(); this.showModal = true; }
  editTopic(topic: OOPTopic) { this.editingTopic = topic; this.formData = { ...topic }; this.showModal = true; }
  closeModal() { this.showModal = false; }

  saveTopic() {
    if (!this.formData.title || !this.formData.category) return;
    const topic = this.formData as OOPTopic;
    if (this.editingTopic) this.api.updateOOPTopic(this.editingTopic.id!, topic).subscribe(() => { this.loadTopics(); this.closeModal(); });
    else this.api.createOOPTopic(topic).subscribe(() => { this.loadTopics(); this.closeModal(); });
  }

  deleteTopic(id: number) { if (confirm('Delete?')) this.api.deleteOOPTopic(id).subscribe(() => this.loadTopics()); }
  toggleFavorite(topic: OOPTopic) { this.api.toggleOOPFavorite(topic.id!).subscribe(u => { topic.isFavorite = u.isFavorite; if (this.showFavoritesOnly && !u.isFavorite) this.topics = this.topics.filter(t => t.id !== topic.id); }); }
  seedTopics() { this.api.seedOOPTopics().subscribe({ next: r => { alert(r.message); this.loadTopics(); }, error: e => alert(e.error || 'Failed') }); }
  openReviewModal(topic: OOPTopic) { this.selectedTopic = topic; this.reviewData = { status: topic.status, confidenceLevel: topic.confidenceLevel }; this.showReviewModal = true; }
  closeReviewModal() { this.showReviewModal = false; }
  saveReview() { if (!this.selectedTopic) return; this.api.updateOOPTopic(this.selectedTopic.id!, { ...this.selectedTopic, ...this.reviewData }).subscribe(() => { this.loadTopics(); this.closeReviewModal(); }); }

  openLessonModal(topic: OOPTopic) { this.lessonTopic = topic; this.showLessonModal = true; }
