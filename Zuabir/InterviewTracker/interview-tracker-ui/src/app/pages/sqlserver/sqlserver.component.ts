...
import { ApiService, SqlServerTopic } from '../../services/api.service';
...
@Component({
  ...
  template: `
    <div>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <h1 style="font-size: 1.75rem; font-weight: 700;">SQL Server Topics</h1>
        <div style="display: flex; gap: 0.5rem;">
          @if (topics.length === 0) { <button class="btn btn-success" (click)="seedTopics()">Load SQL Server Topics with Lessons</button> }
      ...
      @if (topics.length === 0) {
        <div class="card"><div class="empty-state"><div class="empty-state-icon">&#128451;</div><div>No topics. Click "Load SQL Server Topics with Lessons" to start!</div></div></div>
      } @else {
      ...
      @if (showModal) {
        <div class="modal-backdrop" (click)="closeModal()"><div class="modal" (click)="$event.stopPropagation()">
          <h2 class="modal-title">{{ editingTopic ? 'Edit' : 'Add' }} SQL Server Topic</h2>
          <form (ngSubmit)="saveTopic()">
            ...
            <div class="form-group"><label class="form-label">SQL Example</label><textarea class="form-textarea" [(ngModel)]="formData.sqlExample" name="sqlExample" rows="3" style="font-family: monospace;"></textarea></div>
        ...
        <div class="modal-backdrop" (click)="closeLessonModal()">
          <div class="modal lesson-modal" (click)="$event.stopPropagation()" style="max-width: 900px; max-height: 90vh; overflow: hidden; display: flex; flex-direction: column;">
            ...
            <div style="overflow-y: auto; flex: 1; padding: 1rem 0;">
              ...
              @if (lessonTopic.sqlExample) {
                <div style="margin-top: 2rem;">
                  <h3 style="color: #dc2626; margin-bottom: 1rem;">SQL Example</h3>
                  <pre style="background: #1e293b; padding: 1rem; border-radius: 8px; overflow-x: auto; font-size: 0.875rem; line-height: 1.5;"><code>{{ lessonTopic.sqlExample }}</code></pre>
                </div>
              }
            </div>
          </div>
  ...
  styles: [`
    ...
    .lesson-content h1 { font-size: 1.75rem; font-weight: 700; margin: 1.5rem 0 1rem; color: #ef4444; }
    .lesson-content h2 { font-size: 1.4rem; font-weight: 600; margin: 1.5rem 0 0.75rem; color: #f87171; }
    .lesson-content h3 { font-size: 1.15rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: #fca5a5; }
...
export class SqlserverComponent implements OnInit {
  topics: SqlServerTopic[] = []; allTopics: SqlServerTopic[] = []; showModal = false; showReviewModal = false; showLessonModal = false;
  editingTopic: SqlServerTopic | null = null; selectedTopic: SqlServerTopic | null = null; lessonTopic: SqlServerTopic | null = null; showFavoritesOnly = false;
  filters = { category: '', status: '' };
  allCategories = ['Queries', 'DML', 'Indexes', 'Performance', 'Programmability', 'Transactions', 'Schema', 'Advanced', 'Administration'];
  formData: Partial<SqlServerTopic> = this.getEmptyForm(); reviewData = { status: 'Understood', confidenceLevel: 3 };

  constructor(private api: ApiService) {}
  ngOnInit() { this.loadTopics(); }
  loadTopics() { this.api.getSqlServerTopics(this.filters).subscribe(t => { this.allTopics = t; this.topics = this.showFavoritesOnly ? t.filter(x => x.isFavorite) : t; }); }
  ...
  getEmptyForm(): Partial<SqlServerTopic> { return { title: '', category: '', difficulty: 'Medium', status: 'NotStarted', confidenceLevel: 1, keyConcepts: '', sqlExample: '', tags: [], isFavorite: false }; }

  openModal() { this.editingTopic = null; this.formData = this.getEmptyForm(); this.showModal = true; }
  editTopic(t: SqlServerTopic) { this.editingTopic = t; this.formData = { ...t }; this.showModal = true; }
  closeModal() { this.showModal = false; }
  saveTopic() { if (!this.formData.title || !this.formData.category) return; const t = this.formData as SqlServerTopic; if (this.editingTopic) this.api.updateSqlServerTopic(this.editingTopic.id!, t).subscribe(() => { this.loadTopics(); this.closeModal(); }); else this.api.createSqlServerTopic(t).subscribe(() => { this.loadTopics(); this.closeModal(); }); }
  deleteTopic(id: number) { if (confirm('Delete?')) this.api.deleteSqlServerTopic(id).subscribe(() => this.loadTopics()); }
  toggleFavorite(t: SqlServerTopic) { this.api.toggleSqlServerFavorite(t.id!).subscribe(u => { t.isFavorite = u.isFavorite; if (this.showFavoritesOnly && !u.isFavorite) this.topics = this.topics.filter(x => x.id !== t.id); }); }
  seedTopics() { this.api.seedSqlServerTopics().subscribe({ next: r => { alert(r.message); this.loadTopics(); }, error: e => alert(e.error || 'Failed') }); }
  openReviewModal(t: SqlServerTopic) { this.selectedTopic = t; this.reviewData = { status: t.status, confidenceLevel: t.confidenceLevel }; this.showReviewModal = true; }
  closeReviewModal() { this.showReviewModal = false; }
  saveReview() { if (!this.selectedTopic) return; this.api.updateSqlServerTopic(this.selectedTopic.id!, { ...this.selectedTopic, ...this.reviewData }).subscribe(() => { this.loadTopics(); this.closeReviewModal(); }); }

  openLessonModal(t: SqlServerTopic) { this.lessonTopic = t; this.showLessonModal = true; }
