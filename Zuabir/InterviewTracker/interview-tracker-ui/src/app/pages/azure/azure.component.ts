...
import { ApiService, AzureTopic } from '../../services/api.service';
...
@Component({
  ...
  template: `
    <div>
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        <h1 style="font-size: 1.75rem; font-weight: 700;">Azure Cloud Topics</h1>
        <div style="display: flex; gap: 0.5rem;">
          @if (topics.length === 0) { <button class="btn btn-success" (click)="seedTopics()">Load Azure Topics with Lessons</button> }
      ...
      @if (topics.length === 0) {
        <div class="card"><div class="empty-state"><div class="empty-state-icon">&#9729;</div><div>No topics. Click "Load Azure Topics with Lessons" to start!</div></div></div>
      } @else {
            ...
            <div class="card">
              <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                <div style="flex: 1;">
                  ...
                  @if (topic.azureService) { <span style="font-size: 0.75rem; color: #0ea5e9; margin-left: 0.5rem;">{{ topic.azureService }}</span> }
                </div>
      ...
      @if (showModal) {
        <div class="modal-backdrop" (click)="closeModal()"><div class="modal" (click)="$event.stopPropagation()">
          <h2 class="modal-title">{{ editingTopic ? 'Edit' : 'Add' }} Azure Topic</h2>
          <form (ngSubmit)="saveTopic()">
            ...
            <div class="grid grid-2">
              ...
              <div class="form-group"><label class="form-label">Azure Service</label><input type="text" class="form-input" [(ngModel)]="formData.azureService" name="azureService"></div>
            </div>
            ...
            <div style="overflow-y: auto; flex: 1; padding: 1rem 0;">
              ...
              @if (lessonTopic.codeExample) {
                <div style="margin-top: 2rem;">
                  <h3 style="color: #0ea5e9; margin-bottom: 1rem;">Code Example</h3>
  ...
  styles: [`
    ...
    .lesson-content h1 { font-size: 1.75rem; font-weight: 700; margin: 1.5rem 0 1rem; color: #38bdf8; }
    .lesson-content h2 { font-size: 1.4rem; font-weight: 600; margin: 1.5rem 0 0.75rem; color: #7dd3fc; }
    .lesson-content h3 { font-size: 1.15rem; font-weight: 600; margin: 1.25rem 0 0.5rem; color: #bae6fd; }
...
export class AzureComponent implements OnInit {
  topics: AzureTopic[] = []; allTopics: AzureTopic[] = []; showModal = false; showReviewModal = false; showLessonModal = false;
  editingTopic: AzureTopic | null = null; selectedTopic: AzureTopic | null = null; lessonTopic: AzureTopic | null = null; showFavoritesOnly = false;
  filters = { category: '', status: '' };
  allCategories = ['Compute', 'Storage', 'Database', 'Networking', 'Identity', 'Security', 'Integration', 'DevOps', 'Monitoring', 'AI', 'Analytics', 'Governance'];
  formData: Partial<AzureTopic> = this.getEmptyForm(); reviewData = { status: 'Understood', confidenceLevel: 3 };

  constructor(private api: ApiService) {}
  ngOnInit() { this.loadTopics(); }
  loadTopics() { this.api.getAzureTopics(this.filters).subscribe(t => { this.allTopics = t; this.topics = this.showFavoritesOnly ? t.filter(x => x.isFavorite) : t; }); }
  ...
  getEmptyForm(): Partial<AzureTopic> { return { title: '', category: '', difficulty: 'Medium', status: 'NotStarted', confidenceLevel: 1, keyConcepts: '', codeExample: '', azureService: '', tags: [], isFavorite: false }; }

  openModal() { this.editingTopic = null; this.formData = this.getEmptyForm(); this.showModal = true; }
  editTopic(t: AzureTopic) { this.editingTopic = t; this.formData = { ...t }; this.showModal = true; }
  closeModal() { this.showModal = false; }
  saveTopic() { if (!this.formData.title || !this.formData.category) return; const t = this.formData as AzureTopic; if (this.editingTopic) this.api.updateAzureTopic(this.editingTopic.id!, t).subscribe(() => { this.loadTopics(); this.closeModal(); }); else this.api.createAzureTopic(t).subscribe(() => { this.loadTopics(); this.closeModal(); }); }
  deleteTopic(id: number) { if (confirm('Delete?')) this.api.deleteAzureTopic(id).subscribe(() => this.loadTopics()); }
  toggleFavorite(t: AzureTopic) { this.api.toggleAzureFavorite(t.id!).subscribe(u => { t.isFavorite = u.isFavorite; if (this.showFavoritesOnly && !u.isFavorite) this.topics = this.topics.filter(x => x.id !== t.id); }); }
  seedTopics() { this.api.seedAzureTopics().subscribe({ next: r => { alert(r.message); this.loadTopics(); }, error: e => alert(e.error || 'Failed') }); }
  openReviewModal(t: AzureTopic) { this.selectedTopic = t; this.reviewData = { status: t.status, confidenceLevel: t.confidenceLevel }; this.showReviewModal = true; }
  closeReviewModal() { this.showReviewModal = false; }
  saveReview() { if (!this.selectedTopic) return; this.api.updateAzureTopic(this.selectedTopic.id!, { ...this.selectedTopic, ...this.reviewData }).subscribe(() => { this.loadTopics(); this.closeReviewModal(); }); }

  openLessonModal(t: AzureTopic) { this.lessonTopic = t; this.showLessonModal = true; }
